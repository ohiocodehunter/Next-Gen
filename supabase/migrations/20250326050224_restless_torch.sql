...
DROP POLICY IF EXISTS "Users can upload their own license files" ON driver_license_files;
DROP POLICY IF EXISTS "Users can view their own license files" ON driver_license_files;
DROP POLICY IF EXISTS "Admins can view all license files" ON driver_license_files;
...
CREATE POLICY "Enable read access for admins to all files"
  ON driver_license_files
  FOR SELECT
  TO authenticated
  USING (
